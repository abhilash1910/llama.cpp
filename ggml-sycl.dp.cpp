...
typedef void (*to_fp32_cuda_t)(const void *__restrict__ x,
                               float *__restrict__ y, int k,
                               dpct::queue_ptr stream);
typedef void (*dot_kernel_k_t)(const void * __restrict__ vx, const int ib, const int iqs, const float * __restrict__ y, float & v);
typedef void (*cpy_kernel_t)(const char * cx, char * cdst);
typedef void (*ggml_cuda_func_t)(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst);
typedef void (*ggml_cuda_op_t)(const ggml_tensor *src0, const ggml_tensor *src1,
                               ggml_tensor *dst, char *src0_ddq_i,
                               float *src0_ddf_i, float *src1_ddf_i,
                               float *dst_ddf_i, int64_t i02, int64_t i01_low,
                               int64_t i01_high, int i1,
                               dpct::queue_ptr &cudaStream_main);
...
typedef struct dpct_type_940308 {
    sycl::half d;           // delta
...
typedef struct dpct_type_105939 {
...
typedef struct dpct_type_256340 {
    sycl::half d;           // delta
...
typedef struct dpct_type_168287 {
    sycl::half2 dm;         // dm.x = delta, dm.y = min
    uint8_t qh[4];          // 5-th bit of quants
...
typedef struct dpct_type_292566 {
    sycl::half d;           // delta
...
typedef struct dpct_type_105484 {
...
typedef struct dpct_type_152956 {
...
typedef struct dpct_type_116668 {
...
#else
typedef struct dpct_type_132377 {
    sycl::half2 dm;            // super-block scale for quantized scales/mins
...
#else
typedef struct dpct_type_133015 {
    sycl::half2 dm;               // super-block scale for quantized scales/mins
...
typedef struct dpct_type_130975 {
...
struct ggml_tensor_extra_gpu {
    ...
    dpct::event_ptr events[GGML_CUDA_MAX_DEVICES];
    std::chrono::time_point<std::chrono::steady_clock>
        events_ct1[16]; // events for synchronizing multiple GPUs
};
...
static dpct::queue_ptr g_cudaStreams_main[GGML_CUDA_MAX_DEVICES] = {
    &dpct::get_default_queue()};

static void add_f32(const float * x, const float * y, float * dst, const int kx, const int ky,
...
static void add_f16_f32_f16(const sycl::half *x, const float *y,
                            sycl::half *dst, const int k,
                            const sycl::nd_item<3> &item_ct1) {
    const int i = item_ct1.get_local_range(2) * item_ct1.get_group(2) +
                  item_ct1.get_local_id(2);
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:146: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
        */
        mean += dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), mean,
                                               mask);
        /*
        ...
        */
        /*
        DPCT1096:147: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
        */
        var +=
            dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), var, mask);
    }

    mean /= ncols;
    var = var / ncols - mean * mean;
    const float inv_var = sycl::rsqrt(var + eps);
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:148: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void dequantize_block_q2_K(const void * __restrict__ vx, float * __restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
static void dequantize_block_q3_K(const void * __restrict__ vx, float * __restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
static void dequantize_block_q4_K(const void * __restrict__ vx, float * __restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
static void dequantize_block_q5_K(const void * __restrict__ vx, float * __restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
static void dequantize_block_q6_K(const void * __restrict__ vx, float * __restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
static void dequantize_mul_mat_vec_q2_k(const void * __restrict__ vx, const float * __restrict__ yy, float * __restrict__ dst, const int ncols, int nrows,
                                        const sycl::nd_item<3> &item_ct1) {

    static_assert(16%K_QUANTS_PER_ITERATION == 0, "16 must be divisible by K_QUANTS_PER_ITERATION");

    const int row = item_ct1.get_group(1) * item_ct1.get_local_range(1) +
                    item_ct1.get_local_id(1);
    if (row > nrows) return;
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:152: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void dequantize_mul_mat_vec_q3_k(const void * __restrict__ vx, const float * __restrict__ yy, float * __restrict__ dst, const int ncols, int nrows,
                                        const sycl::nd_item<3> &item_ct1) {
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:153: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void dequantize_mul_mat_vec_q4_k(const void * __restrict__ vx, const float * __restrict__ yy, float * __restrict__ dst, const int ncols, int nrows,
                                        const sycl::nd_item<3> &item_ct1) {
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:154: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void dequantize_mul_mat_vec_q5_k(const void * __restrict__ vx, const float * __restrict__ yy, float * __restrict__ dst, const int ncols,
                                        const sycl::nd_item<3> &item_ct1) {
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:155: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:156: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:149: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
        ...
        */
        /*
        DPCT1096:150: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void dequantize_block(const void * __restrict__ vx, float * __restrict__ y, const int k,
                             const sycl::nd_item<3> &item_ct1) {
    const int i = item_ct1.get_local_range(2) * item_ct1.get_group(2) +
...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        ...
        sumi = dpct::dp4a(vi0, u[2*i+0], sumi); // SIMD dot product of quantized values
        ...
        sumi = dpct::dp4a(vi1, u[2*i+1], sumi); // SIMD dot product of quantized values
    }
...
#pragma unroll
    for (int i = 0; i < QR2_K; ++i) {
        ...
        sumf_d += d8[i] * (dpct::dp4a(vi, u[i], 0) * (sc & 0xF)); // SIMD dot product
...
#pragma unroll
        for (int i = i0; i < i0 + QI8_1/2; ++i) {
            ...
            sumi_m    = dpct::dp4a(m,    u[i], sumi_m); // multiply sum of q8_1 values with m
        }
...
#pragma unroll
        for (int j = 0; j < QI8_1; ++j) {
            sumi_d = dpct::dp4a((v[j] >> (4*i)) & 0x0F0F0F0F, u[i*QI8_1 + j], sumi_d); // SIMD dot product
        }
...
#pragma unroll
    for (int i = 0; i < QR5_K; ++i) {
        ...
        const int dot1 = dpct::dp4a(v0i, u[2*i+0], dpct::dp4a(v1i, u[2*i+1], 0)); // SIMD dot product
        const int dot2 = dpct::dp4a(0x01010101, u[2*i+0], dpct::dp4a(0x01010101, u[2*i+1], 0)); // sum of u
...
#pragma unroll
        for (int j = 0; j < QI8_1; ++j) {
            sumi_d = dpct::dp4a(v[i*QI8_1 + j], u[i*QI8_1 + j], sumi_d); // SIMD dot product
        }
...
#pragma unroll
        for (int i = i0; i < i0 + 2; ++i) {
            sumi_d.x = dpct::dp4a(v[2*i+0], u[2*i+0], sumi_d.x); // SIMD dot product
            sumi_d.x = dpct::dp4a(v[2*i+1], u[2*i+1], sumi_d.x); // SIMD dot product

            sumi_d.y = dpct::dp4a(v[2*i+4], u[2*i+4], sumi_d.y); // SIMD dot product
            sumi_d.y = dpct::dp4a(v[2*i+5], u[2*i+5], sumi_d.y); // SIMD dot product
        }
...
#pragma unroll
            for (int i = 0; i < mmq_x; i += nwarps) {
                const int col_y_eff = sycl::min(
                    ...
                    (unsigned int)(ncols_y - 1)); // to prevent out-of-bounds
                                                  // memory accesses
...
#pragma unroll
            for (int ids0 = 0; ids0 < mmq_x; ids0 += nwarps * QI8_1) {
                ...
                const int col_y_eff =
                    sycl::min((int)(col_y_0 + ids), (int)(ncols_y - 1));
...
template <bool need_check> static void mul_mat_q4_0(
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {

#if DPCT_COMPATIBILITY_TEMP >= CC_TURING
...
template <bool need_check> static void mul_mat_q5_0(
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
...
template <bool need_check> static void mul_mat_q5_1(
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
...
template <bool need_check> static void mul_mat_q8_0(
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
...
template <bool need_check> static void mul_mat_q2_K(
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
...
template <bool need_check> static void mul_mat_q5_K(
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:157: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:151: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:158: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:159: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void rope_f32(const float * x, float * dst, const int ncols, const float p0,
                                const float p_delta, const int p_delta_rows, const float theta_scale,
                                const sycl::nd_item<3> &item_ct1) {
    const int col = 2 * (item_ct1.get_local_range(2) * item_ct1.get_group(2) +
                         item_ct1.get_local_id(2));
...
static void rope_glm_f32(const float * x, float * dst, const int ncols, const float p, const float block_p, const float theta_scale,
                         const sycl::nd_item<3> &item_ct1) {
    ...
    const float col_theta_scale = sycl::pow<float>(theta_scale, col);
...
static void soft_max_f32(const float * x, float * dst, const int ncols,
                         const sycl::nd_item<3> &item_ct1) {
    const int row = item_ct1.get_local_range(1) * item_ct1.get_group(1) +
                    item_ct1.get_local_id(1);
    ...
    for (int block_start = 0; block_start < ncols; block_start += block_size) {
        ...
        const int i = row*ncols + col;
        const float val = sycl::exp((float)(x[i]));
        tmp += val;
        dst[i] = val;
    }
...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1) {
        ...
        */
        /*
        DPCT1096:160: The right-most dimension of the work-group used in the
        SYCL kernel that calls this function may be less than "32". The function
        "dpct::permute_sub_group_by_xor" may return an unexpected result on the
        CPU device. Modify the size of the work-group to ensure that the value
        of the right-most dimension is a multiple of "32".
...
static void add_f32_cuda(const float *x, const float *y, float *dst,
                         const int kx, const int ky, dpct::queue_ptr stream) {
    const int num_blocks = (kx + CUDA_ADD_BLOCK_SIZE - 1) / CUDA_ADD_BLOCK_SIZE;
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                              sycl::range<3>(1, 1, CUDA_ADD_BLOCK_SIZE),
                          sycl::range<3>(1, 1, CUDA_ADD_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            add_f32(x, y, dst, kx, ky, item_ct1);
        });
}

static void add_f16_f32_f16_cuda(const sycl::half *x, const float *y,
                                 sycl::half *dst, const int k,
                                 dpct::queue_ptr stream) {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, CUDA_ADD_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            add_f16_f32_f16(x, y, dst, k, item_ct1);
        });
}
...
static void mul_f32_cuda(const float *x, const float *y, float *dst,
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                              sycl::range<3>(1, 1, CUDA_MUL_BLOCK_SIZE),
                          sycl::range<3>(1, 1, CUDA_MUL_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            mul_f32(x, y, dst, kx, ky, item_ct1);
        });
}
...
static void rms_norm_f32_cuda(const float *x, float *dst, const int ncols,
    ...
    Adjust the work-group size if needed.
    */
    //int max_work_group_size = stream(dpct::device).get_device().get_info<info::device::max_work_group_size>();
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, nrows) * block_dims, block_dims),
        [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
...
static void dequantize_row_q4_0_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
...
static void dequantize_row_q4_1_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, CUDA_DEQUANTIZE_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            dequantize_block<QK4_1, QR4_1, dequantize_q4_1>(vx, y, k, item_ct1);
        });
}

static void dequantize_row_q5_0_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, CUDA_DEQUANTIZE_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            dequantize_block<QK5_0, QR5_0, dequantize_q5_0>(vx, y, k, item_ct1);
        });
}

static void dequantize_row_q5_1_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, CUDA_DEQUANTIZE_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            dequantize_block<QK5_1, QR5_1, dequantize_q5_1>(vx, y, k, item_ct1);
        });
}

static void dequantize_row_q8_0_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, CUDA_DEQUANTIZE_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            dequantize_block<QK8_0, QR8_0, dequantize_q8_0>(vx, y, k, item_ct1);
        });
}

static void dequantize_row_q2_K_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int nb = k / QK_K;
#if QK_K == 256
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, nb) * sycl::range<3>(1, 1, 64),
                          sycl::range<3>(1, 1, 64)),
        [=](sycl::nd_item<3> item_ct1) {
...
static void dequantize_row_q3_K_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int nb = k / QK_K;
#if QK_K == 256
    stream->parallel_for(
...
static void dequantize_row_q4_K_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int nb = k / QK_K;
    stream->parallel_for(
...
static void dequantize_row_q5_K_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int nb = k / QK_K;
#if QK_K == 256
    stream->parallel_for(
...
static void dequantize_row_q6_K_cuda(const void *vx, float *y, const int k,
                                     dpct::queue_ptr stream) {
    const int nb = k / QK_K;
#if QK_K == 256
    stream->parallel_for(
...
static void dequantize_mul_mat_vec_q4_0_cuda(const void *vx, const dfloat *y,
    ...
    const sycl::range<3> block_nums(1, block_num_y, 1);
...
static void convert_fp16_to_fp32_cuda(const void *vx, float *y, const int k,
                                      dpct::queue_ptr stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, CUDA_DEQUANTIZE_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
            dequantize_block<1, 1, convert_f16>(vx, y, k, item_ct1);
        });
}
...
static void ggml_mul_mat_vec_nc_f16_f32_cuda(
    ...
    */
    stream->parallel_for(
        sycl::nd_range<3>(block_nums * block_dims, block_dims),
        [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
            mul_mat_vec_nc_f16_f32(vx, y, dst, ncols_x, nrows_x, row_stride_x,
                                   channel_stride_x, nchannels_y / nchannels_x,
                                   item_ct1);
        });
}
...
static void rope_f32_cuda(const float *x, float *dst, const int ncols,
                          const int nrows, const float p0, const float p_delta,
                          const int p_delta_rows, const float theta_scale,
                          dpct::queue_ptr stream) {
    GGML_ASSERT(nrows % 2 == 0);
    const sycl::range<3> block_dims(1, 1, 2 * CUDA_ROPE_BLOCK_SIZE);
    ...
    */
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                             rope_f32(x, dst, ncols, p0, p_delta, p_delta_rows,
                                      theta_scale, item_ct1);
                         });
}

static void rope_glm_f32_cuda(const float *x, float *dst, const int ncols,
                              const int nrows, const float p,
                              const float block_p, const float theta_scale,
                              dpct::queue_ptr stream) {
    GGML_ASSERT(nrows % 4 == 0);
    const sycl::range<3> block_dims(1, 1, 4 * CUDA_ROPE_BLOCK_SIZE);
    ...
    */
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                             rope_glm_f32(x, dst, ncols, p, block_p,
                                          theta_scale, item_ct1);
                         });
}
...
static void diag_mask_inf_f32_cuda(const float *x, float *dst,
                                   ...
                                   dpct::queue_ptr stream) {
    const sycl::range<3> block_dims(1, 1, CUDA_DIAG_MASK_INF_BLOCK_SIZE);
    const int block_num_x = (ncols_x + CUDA_DIAG_MASK_INF_BLOCK_SIZE - 1) / CUDA_DIAG_MASK_INF_BLOCK_SIZE;
    const sycl::range<3> block_nums(1, nrows_x, block_num_x);
    /*
...
static void soft_max_f32_cuda(const float *x, float *dst, const int ncols_x,
                              const int nrows_x, dpct::queue_ptr stream) {
    const sycl::range<3> block_dims(1, 1, WARP_SIZE);
    const sycl::range<3> block_nums(1, nrows_x, 1);
    /*
    ...
    */
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1)
                             [[intel::reqd_sub_group_size(32)]] {
                                 soft_max_f32(x, dst, ncols_x, item_ct1);
                             });
}
...
static void *ggml_cuda_pool_malloc(size_t size, size_t *actual_size) try {
    ...
    */
    //CUDA_CHECK((ptr = (void *)sycl::malloc_device(look_ahead_size,
    //                                              dpct::get_default_queue()),
    //            0));
    ptr = (void *)sycl::malloc_device(look_ahead_size,
                                                  dpct::get_default_queue());
...
static void ggml_cuda_pool_free(void *ptr, size_t size) try {
    ...
    You may need to rewrite this code.
    */
    //CUDA_CHECK((sycl::free(ptr, dpct::get_default_queue()), 0));
        ...
        */
        ...
        for (int id = 0; id < g_device_count; ++id) {
            ...
            */
            ...
            /*
            DPCT1005:80: The SYCL device version is different from CUDA Compute
            Compatibility. You may need to rewrite this code.
            */
        ...
        for (int id = 0; id < g_device_count; ++id) {
            /*
            DPCT1093:81: The "id" device may be not the one intended for use.
            ...
            */
            /*
            DPCT1025:84: The SYCL queue is created ignoring the flag and
            priority options.
            */
            //CUDA_CHECK((g_cudaStreams_main[id] =
            //                dpct::get_current_device().create_queue(),
            //            0));
            g_cudaStreams_main[id] =
                            dpct::get_current_device().create_queue();
            ...
            */
            //CUBLAS_CHECK(
            //    (g_cublas_handles[id] = &dpct::get_default_queue(), 0));
            g_cublas_handles[id] = &dpct::get_default_queue();
            /*
            DPCT1027:86: The call to cublasSetMathMode was replaced with 0
...
void *ggml_sycl_host_malloc(size_t size) try {
    ...
    */
    //if (err != 0) {
        ...
        /*
        DPCT1026:90: The call to cudaGetLastError was removed because this call
        ...
        */
        ...
        //    /*
        //    DPCT1009:91: SYCL uses exceptions to report errors and does not use
...
inline void ggml_cuda_op_add(const ggml_tensor *src0, const ggml_tensor *src1,
    ...
    if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        add_f16_f32_f16_cuda((sycl::half *)src0_ddq_i, src1_ddf_i,
                             (sycl::half *)dst_ddf_i, ne00 * i01_diff,
                             cudaStream_main);
    } else {
        GGML_ASSERT(false);
    }
...
inline void ggml_cuda_op_rms_norm(const ggml_tensor *src0,
                                  ...
                                  char *src0_ddq_i, float *src0_ddf_i,
                                  float *src1_ddf_i, float *dst_ddf_i,
                                  int64_t i02, int64_t i01_low,
                                  int64_t i01_high, int i1,
                                  dpct::queue_ptr &cudaStream_main) {
...
inline void ggml_cuda_op_mul_mat_q(const ggml_tensor *src0,
                                   const ggml_tensor *src1, ggml_tensor *dst,
                                   char *src0_ddq_i, float *src0_ddf_i,
                                   float *src1_ddf_i, float *dst_ddf_i,
                                   int64_t i02, int64_t i01_low,
                                   int64_t i01_high, int i1,
                                   dpct::queue_ptr &cudaStream_main) try {
...
inline void ggml_cuda_op_mul_mat_vec(const ggml_tensor *src0,
                                     const ggml_tensor *src1, ggml_tensor *dst,
                                     char *src0_ddq_i, float *src0_ddf_i,
                                     float *src1_ddf_i, float *dst_ddf_i,
                                     int64_t i02, int64_t i01_low,
...
inline void ggml_cuda_op_mul_mat_cublas(
    ...
    char *src0_ddq_i, float *src0_ddf_i, float *src1_ddf_i, float *dst_ddf_i,
    ...
    You may need to rewrite this code.
    */
    g_cublas_handles[id] = cudaStream_main;
    //CUBLAS_CHECK((g_cublas_handles[id] = cudaStream_main, 0));
    //CUBLAS_CHECK(
    //    (/*
    ...
    //         oneapi::mkl::transpose::nontrans, i01_diff, ne11, ne10, alpha,
    //         src0_ddf_i, ne00, src1_ddf_i, ne10, beta, dst_ddf_i, ldc),
    //     0));
    oneapi::mkl::blas::column_major::gemm(
             *g_cublas_handles[id], oneapi::mkl::transpose::trans,
             oneapi::mkl::transpose::nontrans, i01_diff, ne11, ne10, alpha,
...
inline void ggml_cuda_op_rope(const ggml_tensor *src0, const ggml_tensor *src1,
    ...
    if (is_glm) {
        ...
        const float id_p = fminf(p, n_ctx - 2.f);
        const float block_p = fmaxf(p - (n_ctx - 2.f), 0.f);
...
static void ggml_cuda_op(const ggml_tensor *src0, const ggml_tensor *src1,
                         ggml_tensor *dst, ggml_cuda_op_t op,
                         bool src0_needs_f32, bool flatten_rows) try {
    const int64_t ne00 = src0->ne[0];
    const int64_t ne01 = src0->ne[1];
    ...
    if (split && g_device_count > 1) {
        /*
        DPCT1093:101: The "g_main_device" device may be not the one intended for
        ...
        */
        ...
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((dpct::select_device(g_main_device), 0));
        /*
        DPCT1012:103: Detected kernel execution time measurement pattern and
        generated an initial code for time measurements in SYCL. You can change
        the way time is measured depending on your goals.
        */
        /*
        DPCT1024:104: The original code returned the error code that was further
        consumed by the program logic. This original code was replaced with 0.
        You may need to rewrite the program logic consuming the error code.
        */
        src0_extra->events_ct1[g_main_device] =
            std::chrono::steady_clock::now();
        //CUDA_CHECK(
        //    (*src0_extra->events[g_main_device] =
        //         g_cudaStreams_main[g_main_device]->ext_oneapi_submit_barrier(),
        //     0));
        *src0_extra->events[g_main_device] =
                 g_cudaStreams_main[g_main_device]->ext_oneapi_submit_barrier();
    }
    ...
    for (int id = 0; id < g_device_count; ++id) {
        ...
        /*
        DPCT1093:105: The "id" device may be not the one intended for use.
        Adjust the selected device if needed.
        */
        dpct::select_device(id);
        dpct::queue_ptr cudaStream_main = g_cudaStreams_main[id];
        ...
        if (split && id != g_main_device) {
            ...
            inserted. You may need to rewrite this code.
            */
            //CUDA_CHECK((cudaStream_main->ext_oneapi_submit_barrier(
            //                {*src0_extra->events[g_main_device]}),
            //            0));
        ...
        for (int64_t i03 = 0; i03 < i03_max; i03++) {
            ...
            for (int64_t i02 = 0; i02 < i02_max; i02++) {
                ...
                if (use_src1 && !src1_stays_on_host) {
                    if (src1->backend == GGML_BACKEND_CPU) {
                        ...
                        if (id != g_main_device) {
                            ...
                            /*
                            DPCT1003:107: Migrated API does not return error
                            code. (*, 0) is inserted. You may need to rewrite
                            this code.
                            */
                            //CUDA_CHECK((cudaStream_main->memcpy(
                            //                src1_ddf_i, src1_ddf_i_source,
                            //                src1_stride * sizeof(float)),
                            //            0));
                            cudaStream_main->memcpy(
                                            src1_ddf_i, src1_ddf_i_source,
                                            src1_stride * sizeof(float));
                        }
                        ...
                        ggml_cuda_cpy_tensor_2d(src1_ddf_i, src1, i03, i02, 0, ne11, cudaStream_main);
                    } else {
                        GGML_ASSERT(false);
                    }
                }
                ...
                if ((!src0_on_device || !src0_is_contiguous) && i02 % i02_divisor == 0) {
                    if (src0_is_f32) {
                         ggml_cuda_cpy_tensor_2d(src0_ddf_i, src0, i03, i02/i02_divisor, i01_low, i01_high, cudaStream_main);
                        //CUDA_CHECK(ggml_cuda_cpy_tensor_2d(src0_ddf_i, src0, i03, i02/i02_divisor, i01_low, i01_high, cudaStream_main));
                    } else {
                         ggml_cuda_cpy_tensor_2d(src0_ddq_i, src0, i03, i02/i02_divisor, i01_low, i01_high, cudaStream_main);
                ...
                if (src0_needs_f32 && !src0_is_f32) {
                    ...
                    /*
                    DPCT1010:108: SYCL uses exceptions to report errors and does
                ...
                /*
                DPCT1010:109: SYCL uses exceptions to report errors and does not
                ...
                */
                ...
                if (!dst_on_device) {
                    ...
                    if (split) {
                        ...
                        /*
                        DPCT1003:110: Migrated API does not return error code.
                        (*, 0) is inserted. You may need to rewrite this code.
                        */
                        //CUDA_CHECK((dpct::async_dpct_memcpy(
                        //                dhf_dst_i, ne0 * sizeof(float),
                        //                dst_ddf_i, i01_diff * sizeof(float),
                        //                i01_diff * sizeof(float), ne1, kind,
                        //                *cudaStream_main),
                        //            0));
                        ...
                        /*
                        DPCT1003:111: Migrated API does not return error code.
                        (*, 0) is inserted. You may need to rewrite this code.
                        */
                        //CUDA_CHECK((
                        //    cudaStream_main->memcpy(dhf_dst_i, dst_ddf_i,
    ...
    for (int id = 0; id < g_device_count; ++id) {
        ...
        /*
        DPCT1093:114: The "id" device may be not the one intended for use.
        Adjust the selected device if needed.
        */
        /*
        DPCT1003:115: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((dpct::select_device(id), 0));
    ...
    if (split && g_device_count > 1) {
        /*
        DPCT1093:116: The "g_main_device" device may be not the one intended for
        use. Adjust the selected device if needed.
        */
        /*
        DPCT1003:117: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((dpct::select_device(g_main_device), 0));
        for (int id = 0; id < g_device_count; ++id) {
            if (id != g_main_device && src0_extra->events[id]) {
                /*
                DPCT1003:118: Migrated API does not return error code. (*, 0) is
                inserted. You may need to rewrite this code.
                */
                //CUDA_CHECK(
                //    (g_cudaStreams_main[g_main_device]
                //         ->ext_oneapi_submit_barrier({*src0_extra->events[id]}),
                //     0));
                g_cudaStreams_main[g_main_device]
                         ->ext_oneapi_submit_barrier({*src0_extra->events[id]});
            }
    ...
    if (dst->backend == GGML_BACKEND_CPU) {
        /*
        DPCT1093:119: The "g_main_device" device may be not the one intended for
        use. Adjust the selected device if needed.
        */
        /*
        ...
        */
        //CUDA_CHECK((dpct::select_device(g_main_device), 0));
        //dpct::device device = select_device(g_main_device);
        /*
        DPCT1003:121: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((dpct::get_current_device().queues_wait_and_throw(), 0));
...
void ggml_sycl_mul_mat_vec_p021(const ggml_tensor *src0,
    ...
    /*
    DPCT1093:122: The "g_main_device" device may be not the one intended for
    use. Adjust the selected device if needed.
    */
    /*
    ...
    */
    //CUDA_CHECK((dpct::select_device(g_main_device), 0));
    //dpct::device device = select_device(g_main_device);
    dpct::queue_ptr cudaStream_main = g_cudaStreams_main[g_main_device];
...
void ggml_sycl_mul_mat_vec_nc(const ggml_tensor *src0, const ggml_tensor *src1,
    ...
    /*
    DPCT1093:124: The "g_main_device" device may be not the one intended for
    use. Adjust the selected device if needed.
    */
    /*
    ...
    */
    ...
    const int row_stride_x = nb01 / sizeof(sycl::half);
    const int channel_stride_x = nb02 / sizeof(sycl::half);
...
void ggml_cuda_cpy(const ggml_tensor *src0, const ggml_tensor *src1,
                   ggml_tensor *dst) try {
    ...
    /*
    DPCT1093:126: The "g_main_device" device may be not the one intended for
    use. Adjust the selected device if needed.
    */
    /*
...
void ggml_cuda_transform_tensor(void *data, struct ggml_tensor *tensor) try {
    ...
    for (int id = 0; id < g_device_count; ++id) {
        ...
        /*
        DPCT1093:128: The "id" device may be not the one intended for use.
        Adjust the selected device if needed.
        */
        dpct::select_device(id);
        ...
        */
        //CUDA_CHECK(
        //    (buf = (char *)sycl::malloc_device(size, dpct::get_default_queue()),
        //     0));
        buf = (char *)sycl::malloc_device(size, dpct::get_default_queue());
        ...
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((dpct::get_default_queue()
        ...
        if (backend == GGML_BACKEND_GPU_SPLIT) {
            ...
            inserted. You may need to rewrite this code.
            */
            //CUDA_CHECK((extra->events[id] = new sycl::event(), 0));
            extra->events[id] = new sycl::event();
        }
    }
...
void ggml_sycl_free_data(struct ggml_tensor *tensor) try {
    ...
    for (int id = 0; id < g_device_count; ++id) {
        if (extra->data_device[id] != nullptr) {
            /*
            DPCT1093:133: The "id" device may be not the one intended for use.
            Adjust the selected device if needed.
            */
            /*
        ...
        if (extra->events[id] != nullptr) {
            /*
            DPCT1093:136: The "id" device may be not the one intended for use.
            Adjust the selected device if needed.
            */
            /*
            ...
            */
            ...
            inserted. You may need to rewrite this code.
            */
            //CUDA_CHECK((dpct::destroy_event(extra->events[id]), 0));
            dpct::destroy_event(extra->events[id]);
        }
    }
...
void ggml_sycl_assign_buffers_impl(struct ggml_tensor *tensor, bool scratch,
    ...
    /*
    DPCT1093:139: The "g_main_device" device may be not the one intended for
    use. Adjust the selected device if needed.
    */
    /*
    ...
    */
    ...
    if (inplace && (tensor->src[0]->backend == GGML_BACKEND_GPU || tensor->src[0]->backend == GGML_BACKEND_GPU_SPLIT)) {
        ...
        if (data == nullptr) {
            /*
            DPCT1003:141: Migrated API does not return error code. (*, 0) is
            inserted. You may need to rewrite this code.
            */
            //CUDA_CHECK((data = (char *)sycl::malloc_device(
            //                g_scratch_size, dpct::get_default_queue()),
            //            0));
        ...
        void * data;
        /*
        DPCT1003:142: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((
        //    data = (void *)sycl::malloc_device(size, dpct::get_default_queue()),
        //    0));
        /*
...
void ggml_sycl_set_main_device(int main_device) try {
    ...
    if (g_device_count > 1) {
        dpct::device_info prop;
        /*
        DPCT1003:144: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        //CUDA_CHECK((dpct::dev_mgr::instance()
        //                .get_device(g_main_device)
...
void ggml_sycl_free_scratch() try {
    ...
    You may need to rewrite this code.
    */
    //CUDA_CHECK((sycl::free(g_scratch_buffer, dpct::get_default_queue()), 0));
